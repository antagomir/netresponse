...
\author{Leo Lahti$1,2$\footnote{leo.lahti@iki.fi}, Olli-Pekka Huovilainen$1$, Ant{\'o}nio
Gusm{\~a}o$1$ and Juuso Parkkinen$1$\\
...
ICMg is used to identify community structure in interaction networks; 
NetResponse is used to detect and characterize subnetworks that
exhibit context-specific activation patterns across versatile
collections of functional measurements, such as gene expression data.
The implementations are partially based on the agglomerative
independent variable group analysis \citep{Honkela08} and variational
Dirichlet process Gaussian mixture models \cite{Kurihara07nips}. 

%The package provides also implementations for aivga and vdp.
%Further tools %for visualization and analysis will be provided in the
%later %versions. 
