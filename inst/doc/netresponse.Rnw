...
\title{netresponse}
\author{Leo Lahti, Olli-Pekka Huovilainen, Ant{\'o}nio Gusm{\~a}o
and Juuso Parkkinen}
...
in the data matrix D. Various network formats are supported, see
help(detect.responses) for details:
...
From version 1.3.35, the package contains a method called Interaction
Component Model for finding functional gene modules \cite{Parkkinen10bmcsysbio}
from either protein interaction data or from combinations of protein 
