...
list.responses.factor.minimal <- function(annotation.df, groupings, method = "hypergeometric", 
    ...
    for (fnam in colnames(annotation.df)) {
        ...
        responses.per.level <- factor.responses.minimal(annotation.vector, groupings, 
            method = method, min.size = min.size, data = data)
        ...
        if (length(responses.per.level) > 0 && !is.null(na.omit(names(responses.per.level)))) {
            for (level in na.omit(names(responses.per.level))) {
                ...
                colnames(responses.per.level[[level]]) <- c(colnames(tmp)[seq_len(ncol(tmp) - 
                  2)], "Factor", "Level")
            ...
            if (length(responses.per.level) > 1) {
                for (i in 2:length(responses.per.level)) {
                  mat <- rbind(mat, responses.per.level[[i]])
                }
            }
            
        }
        
        # FIXME: do.call(rbind, l) tai do.call(cbind, l) is a neater way to implement
        # this
    ...
    # Order by pvalue
    collected.table <- collected.table[order(as.numeric(as.character(collected.table[, 
        "pvalue"]))), ]
    ...
    if (!is.null(collected.table)) {
        ...
        if (nrow(collected.table) > 100) {
            collected.table$p.adj <- qvalue(collected.table$pvalue, gui = FALSE, 
                fdr.level = 0.25)$qvalue
            p.adj.method <- "qvalue"
        } else {
    ...
    if (!is.null(rounding)) {
        ...
        collected.table$fraction.in.subset <- round(collected.table$fraction.in.subset, 
            rounding)
        collected.table$fraction.in.data <- round(collected.table$fraction.in.data, 
            rounding)
