...
#' 
read.sif <- function(sif.file, format = "graphNEL", directed = FALSE, header = TRUE, 
    sep = "\t", ...) {
...
# g <- read.graph(file, format = c('edgelist', 'pajek', 'ncol', 'lgl', 'graphml',
# 'dimacs', 'graphdb', 'gml')) FIXME: check also these; then convert graph to
# graphNEL
