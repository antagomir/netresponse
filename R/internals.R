Pgm <- function (dat, pars) {

  # Probability density on each data point for each mode
  # no normalization between modes or data points
  
  # modes x samples
  ptm <- matrix(NA, nrow = nrow(pars$means), ncol = ncol(dat))
  colnames(ptm) <- colnames(dat)

  for (mode in 1:length( pars$ws )) {
    ptm[mode, ] <- apply(apply(dat, 2, function( x ){
      dnorm(x, mean = unlist(pars$means[mode, ]),
                 sd = unlist(pars$sds[mode, ]))}),
                         2, prod)
  }

  # probability density can never be zero avoid floating point errors
  # by assigning extremely low probability on those that get
  # zero. here use the smallest probability assigned to
  # non.zero-prob. points if this is smaller than our extreme point
  ptm[ptm == 0] <- min(min(ptm[ptm>0]), 1e-323)
