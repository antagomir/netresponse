...
list.responses.factor <- function(annotation.df, models, method = "hypergeometric", 
    ...
    for (fnam in colnames(annotation.df)) {
        ...
        responses.per.level <- factor.responses(annotation.vector, models, method = method, 
            min.size = min.size, data = data)
    ...
    if (!is.null(collected.table)) {
        ...
        if (nrow(collected.table) > 100) {
            collected.table$qvalue <- qvalue(collected.table$pvalue, gui = FALSE, 
