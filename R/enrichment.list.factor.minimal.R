...
enrichment.list.factor.minimal <- function(groupings, method, verbose = FALSE, annotation.vector, 
    ...
    for (subnet.id in names(groupings)) {
        ...
        for (response in names(groupings.list)) {
            ...
            # In enrichment calculations use only those samples where annotations are
            # available
            ...
            enr <- response.enrichment(total.samples, response.samples, level.samples, 
                method)
